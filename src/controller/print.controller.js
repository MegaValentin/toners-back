  ...
  try {
    ...
    if (toner) {
      ...
      if (!tonerExists) {
        return res
          .status(404)
          .json({ message: "El toner especificado no existe" });
      }
      tonerName = tonerExists.toner;
    }
...
export const paginatedAndFilter = async (req, res) => {
  try {
    const {
      page = 1,
      limit = 10,
    ...
    if (marca) {
      filters.marca = { $regex: search, $options: "i" };
    }
    if (modelo) {
      filters.modelo = { $regex: search, $options: "i" };
    }
    if (toner) {
      filters.toner = toner;
    }

    const total = await Print.countDocuments(filters);

    const prints = await Print.find(filters)
      .populate("toner", "toner") // solo el nombre del toner
